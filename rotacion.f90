...
program campoescalar
         ...
         open(unit=1, file="campoescalarfeb23.dat")

         do i = 1, 15
         u0 = tan((i*5d0*pi)/180d0)

          print*, "entrara a buscar la eigenenergia para:", u0
          w =  buscarE(rlim, h, w0, u0)

         print*, "el valor de W que se mandara a la subrutina es :", w

             call RK4(rlim, h, w, u0)
             write(1,*)  "      "
           end do
...
do while (r<=rlim)
          ...
          ! if ( floor(imp*1d1) == floor(f0*1d1) ) then
           ...
           c=c+1
                 ...
                 l1 = h*g(r, nu, ld, phi, u,w,B)
                 knu1 = h*fnu(r, nu, ld, phi, u,w,B)
                 kld1 = h*fld(r, nu, ld, phi, u,w,B)
                 !!!!!!!!!!!!!!!!rotacion!!!!!!!!!!!
                 kb1  = h*X(uB)
                 lb1  = h*Y(r, nu, ld, phi, u,w,B,uB)


                 k2 = h*f(r+0.5d0*h, nu+0.5d0*knu1,ld+0.5d0*kld1,phi+0.5d0*k1,u+0.5d0*l1,w)
                 l2 = h*g(r+0.5d0*h, nu+0.5d0*knu1,ld+0.5d0*kld1,phi+0.5d0*k1,u+0.5d0*l1,w,B+0.5*kb1)
                 knu2 = h*fnu(r+0.5d0*h, nu+0.5d0*knu1, ld+0.5d0*kld1, phi+0.5d0*k1,u+0.5d0*l1,w,B+0.5*kb1)
                 kld2 = h*fld(r+0.5d0*h, nu+0.5d0*knu1, ld+0.5d0*kld1, phi+0.5d0*k1,u+0.5d0*l1,w,B+0.5*kb1)
                 !!!!!!!!!!!!!!!!rotacion!!!!!!!!!!!
                 kb2  = h*X(uB+0.5*lb1)
                 lb2  = h*Y(r+0.5d0*h, nu+0.5d0*knu1, ld+0.5d0*kld1, phi+0.5d0*k1,u+0.5d0*l1,w,B+0.5*kb1,uB+0.5*lb1)


                 k3 = h*f(r+0.5d0*h, nu+0.5d0*knu2,ld+0.5d0*kld2, phi+0.5d0*k2,  u+0.5d0*l2,w)
                 l3 = h*g(r+0.5d0*h, nu+0.5d0*knu2, ld+0.5d0*kld2, phi+0.5d0*k2,  u+0.5d0*l2,w,B+0.5*kb2)
                 knu3 = h*fnu(r+0.5d0*h, nu+0.5d0*knu2, ld+0.5d0*kld2, phi+0.5d0*k2,u+0.5d0*l2,w,B+0.5*kb2)
                 kld3 = h*fld(r+0.5d0*h, nu+0.5d0*knu2, ld+0.5d0*kld2, phi+0.5d0*k2,  u+0.5d0*l2,w,B+0.5*kb2)
                 !!!!!!!!!!!!!!!!rotacion!!!!!!!!!!!
                 kb3  = h*X(uB+0.5*lb2)
                 lb3  = h*Y(r+0.5d0*h, nu+0.5d0*knu2, ld+0.5d0*kld2, phi+0.5d0*k2,  u+0.5d0*l2,w,B+0.5*kb2,uB+0.5*lb2)


                 k4 = h*f(r+0.5d0*h, nu+knu3, ld+kld3, phi+k3, u+l3,w)
                 l4 = h*g(r+0.5d0*h, nu+knu3, ld+kld3, phi+k3, u+l3,w,B+kb3)
                 knu4 = h*fnu(r+0.5d0*h, nu+knu3, ld+kld3, phi+k3, u+l3,w,B+kb3)
                 kld4 = h*fld(r+0.5d0*h, nu+knu3, ld+kld3, phi+k3, u+l3,w,B+kb3)
                 !!!!!!!!!!!!!!!!rotacion!!!!!!!!!!!
                 kb4  = h*X(uB+lb3)
                 lb4  = h*Y(r+0.5d0*h, nu+knu3, ld+kld3, phi+k3, u+l3,w,B+kb3,uB+lb3)
...
real(kind = 8) function RK4test(rlim,h, w, u0, ind)
           use inicon
           implicit none
...
real(kind = 8) function buscarE(rlim, h, w0, u0)
           ...
           !print*, wp
                     ...
                     phip = RK4test(rlim,h, wp, u0, 0)
                     ii = RK4test(rlim,h, wp, u0, 1)
                     print*, wp, phip
                     cont = 0
                     contaux = 0
...
real(kind = 8) function g(r, nu, ld, phi,u,w,B)
  use inicon
  implicit none
  ...
  if ( r==0 ) then
            ...
            g=u*((fld(r, nu, ld, phi,u,w,B)/ld)-(fnu(r, nu, ld, phi,u,w,B)/nu)-(2.0d0/r)) + phi*(ld**2)*(1.0d0-((w/nu)**2))&
...
real(kind = 8) function Y(r, nu, ld, phi,u,w,B,uB)
  use inicon
  implicit none
  real(8)::  r, nu, ld, phi,u,w,B,uB
  real(kind=8), external :: fnu, fld

  if ( r == 0d0) then
    Y = (ml*phi**2)*(B0*ml-w)
  elseif (r> 0d0 .and. r .lt. 100000d0) then
    Y = uB*(fld(r, nu, ld, phi,u,w,B)/ld+fnu(r, nu, ld, phi,u,w,B)/nu-4d0/r) &
...
real(kind = 8) function fnu(r, nu, ld, phi,u,w,B)
...
real(kind = 8) function fld(r, nu, ld, phi,u,w,B)
  use inicon
  implicit none
  real(8):: r, nu, ld, phi,u,w,B
